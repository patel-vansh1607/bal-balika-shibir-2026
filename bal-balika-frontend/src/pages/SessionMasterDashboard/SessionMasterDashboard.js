...
export default function SessionMasterDashboard({ activeRegion }) {
    ...
    const fetchMasterAnalytics = async () => {
      try {
        ...
        let attendeesQuery = supabase
          .from("attendees")
          .select("id, member_id, region, center");

        if (!isGlobal) {
          const activePrefix = REGION_PREFIXES[activeRegion];
          if (activePrefix) {
            attendeesQuery = attendeesQuery.like(
              "member_id",
              `${activePrefix}%`,
            );
          }
        ...
        const localAttendeeIds = new Set(
          (rosterPool || []).map((a) => String(a.id).trim()),
        );
        ...
        const computedSummary = baseSessions.map((session) => {
          const sessionMatches = rawLogs.filter((log) => {
            ...
            return isGlobal
              ? true
              : localAttendeeIds.has(String(log.attendee_id).trim());
          });
        ...
        if (
          !targetSessionId ||
          !baseSessions.some((s) => s.id === targetSessionId)
        ) {
          targetSessionId = baseSessions[0].id;
          setSelectedSessionId(targetSessionId);
        }

        const targetSessionLogs = rawLogs.filter(
          (log) => log.session_id === targetSessionId,
        );
        const checkedInIdsSet = new Set(
          targetSessionLogs.map((l) => String(l.attendee_id).trim()),
        );
        ...
        const rankedBreakdown = Object.values(groupsMap)
          .map((g) => {
            const percentage =
  ...
  const focusedSessionData = sessionsSummary.find(
    ...
    <div className={styles.masterContainer}>
      <div className={styles.dashHeader}>
        ...
        <p>
          ...
          <strong>{isGlobal ? "Global African Database" : activeRegion}</strong>
        </p>
        {focusedSessionData.title && (
          <p className={styles.activeSubFocus}>
            Data for: <span>{focusedSessionData.title}</span>
          </p>
        )}
      </div>
            ...
            return (
              ...
              >
                <div className={styles.cardHeader}>
                  <div>
                    <h4>
                      {session.title}{" "}
                      {isTargetFocus && (
                        <span className={styles.activeLabelDot}>
                          ● View Data
                        </span>
                      )}
                    </h4>
                ...
                <div className={styles.cardStatsBar}>
                  ...
                  <div className={styles.statMiniBox}>
                    <strong className={styles.textPresent}>
                      {session.present}
                    </strong>
                    <span>Checked-In</span>
                  </div>
                  <div className={styles.statMiniBox}>
                    <strong className={styles.textAbsent}>
                      {session.absent}
                    </strong>
                    <span>Absent</span>
                  </div>
        ...
        <div className={styles.analyticsSidebar}>
          ...
          <div className={styles.sidebarCard}>
                    ...
                    <div className={styles.miniTrackBar}>
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor:
                            item.percentage === 100 ? "#137333" : "#8a151b",
                        }}
                      />
                    </div>
                    <span className={styles.rankPercentageTag}>
                      {item.percentage}%
                    </span>
                  </div>
                </div>
            ...
            <div className={styles.quickTotalsBar}>
              <div className={styles.totalMiniItem}>
                <FaUsers />{" "}
                <span>
                  Exp: <strong>{focusedSessionData.total}</strong>
                </span>
              </div>
              <div className={styles.totalMiniItem}>
                <FaCheckCircle className={styles.textPresent} />{" "}
                <span>
                  In:{" "}
                  <strong className={styles.textPresent}>
                    {focusedSessionData.present}
                  </strong>
                </span>
              </div>
            </div>
