  ...
  useEffect(() => {
    const fetchSessionWorkspace = async () => {
      try {
        ...
        // Resolve baseline registry via member_id prefix rules
        ...
        const attendeeLookupMap = new Map(scopedRoster.map(a => [String(a.id).trim(), a]));

        // Fetch verification gate pass logs matrix row records
        ...
        const logsCheckedInIds = new Set(logsData.map((log) => String(log.attendee_id).trim()));

        // Filter out records based on structural boundary checks
        let filteredRoster = scopedRoster;
        if (!isGlobal && activePrefix) {
          filteredRoster = scopedRoster.filter(a => a.member_id && a.member_id.startsWith(activePrefix));
        }

        let parsedRosterStatus = filteredRoster.map((attendee) => {
          const attendeeKey = String(attendee.id).trim();
          const matchingLog = logsData.find((log) => String(log.attendee_id).trim() === attendeeKey);
        ...
        // Safe Append For Dynamic External Scans Intercepted
        const untrackedLogEntries = logsData.filter(log => !attendeeLookupMap.has(String(log.attendee_id).trim()));
        ...
        if (untrackedLogEntries.length > 0) {
          ...
          untrackedLogEntries.forEach((log) => {
            ...
            const profileMatch = externalProfiles?.find(p => String(p.id).trim() === cleanLogId);
            
            // Apply filtering condition even onto appended stray logs when outside global mode
            if (!isGlobal && activePrefix && profileMatch && !profileMatch.member_id?.startsWith(activePrefix)) {
              return; 
            }

            const alreadyInjected = parsedRosterStatus.some(item => String(item.id).trim() === cleanLogId);
