...
export default function SessionDataDetails() {
    ...
    const fetchSessionDataPool = async () => {
      try {
        ...
        let rosterQuery = supabase
          .from("attendees")
          .select("id, member_id, name, region, center");
        ...
        if (logPool && logPool.length > 0) {
          ...
          completeLogs = logPool
            .map((log) => {
              const profile = attendeesInfo.find(
                (a) => String(a.id).trim() === String(log.attendee_id).trim(),
              );
              return {
                ...log,
        ...
        const checkedInIdsSet = new Set(
          completeLogs.map((l) => String(l.attendee_id).trim()),
        );
        const unverifiedSaints = baselineRoster.filter(
          (rosterMember) =>
            !checkedInIdsSet.has(String(rosterMember.id).trim()),
        );
        setAbsentList(unverifiedSaints);
      } catch (err) {
        console.error(
          "Error processing stream parameters matrix detail logs:",
          err.message,
        );
      } finally {
        setLoading(false);
  ...
  const exportToPDF = () => {
    try {
      ...
      doc.text(
        `Region: ${isGlobal ? "Global African Network" : activeRegion}`,
        14,
        30,
      );
      ...
      if (isGlobal) {
        ...
        const sortedGlobalLogs = [...logs].sort((a, b) =>
        ...
        const sortedLocalLogs = [...logs].sort((a, b) =>
      ...
      if (isGlobal) {
        ...
        const sortedGlobalAbsent = [...absentList].sort((a, b) =>
          (a.region || "").localeCompare(b.region || ""),
        );
        ...
        const sortedLocalAbsent = [...absentList].sort((a, b) =>
          (a.center || "").localeCompare(b.center || ""),
        );
      ...
      autoTable(doc, {
        ...
        body:
          absentRows.length === 0
            ? [["-", "All expected records successfully checked in.", "-", ""]]
            : absentRows,
        theme: "striped",
        headStyles: {
          fillColor: [138, 21, 27],
        ...
        didDrawPage: function (data) {
          ...
          doc.text(
            `Page ${data.pageNumber} of Attendance Report`,
            14,
      ...
      console.error(
        "PDF Export generation pipeline sequence failure exception:",
        err,
      );
    } finally {
  ...
  return (
    <div className={styles.masterContainer}>
      ...
      <div className={styles.detailHeaderWrapper}>
        <div className={styles.headerLeft}>
          ...
          <h1 className={styles.detailMainTitle}>
            {sessionMeta?.title || "Session Activity Hub"}
          </h1>
          <p className={styles.detailSubtitle}>
            Region:{" "}
            <strong>
              {isGlobal ? "Global African Network" : activeRegion}
            </strong>
          </p>
        </div>
        ...
        <div className={styles.headerRight}>
          ...
          >
            ...
            {isGlobal
              ? "Export Region Audit Report PDF"
              : "Export Center Ledger PDF"}
          </button>
        </div>
      </div>
      ...
      <div className={styles.statsHorizontalGrid}>
        <div className={styles.metricRowCard}>
          <div
            ...
            style={{ color: "#2d2926", background: "#f5f2ef" }}
          >
            <FaUsers />
          </div>
          <div>
        ...
        <div className={styles.metricRowCard}>
          <div
            className={styles.metricIconBox}
            style={{ color: "#8a151b", background: "#fde8e8" }}
          >
            <FaUserClock />
          </div>
          <div>
      ...
      >
        ...
        >
          <FaTimesCircle style={{ color: "#8a151b" }} />
          Absent/Pending
        </h3>
              ...
              {absentList.length === 0 ? (
                <tr>
                  <td
                    ...
                    style={{ color: "#137333" }}
                  >
                    Perfect Score! All have successfully checked into this
                    session.
                  </td>
                </tr>
              ) : (
                absentList.map((member) => (
                  <tr key={member.id}>
                    <td className={styles.tablePrimaryCell}>
                      <code>{member.member_id || "N/A"}</code>
                    </td>
                    <td>{member.name || "Unknown Saint"}</td>
                    <td>{member.region || "N/A"}</td>
                    <td>
                      <span
                        className={styles.centerBadgeTag}
                        style={{ background: "#fef3f3", color: "#8a151b" }}
                      >
                        {member.center || "N/A"}
                      </span>
                    </td>
