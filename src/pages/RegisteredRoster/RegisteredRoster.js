...
const handleEditProfile = (attendee) => {
  ...
  const isSpecialRegion = ["Botswana", "South Africa", "Malawi", "Zambia"].includes(attendeeCountry);
...
const handleSaveProfile = async (e) => {
  ...
  try {
    const updatePayload = {
      ...
      name: `${editingAttendee.first_name} ${editingAttendee.middle_name} ${editingAttendee.last_name}`.replace(/\s+/g, ' ').trim(),
      ...
      prev.map((item) =>
        item.id === editingAttendee.id 
          ? { 
              ...
              ...(editingAttendee.isSpecialRegion && { tshirt_size: updatePayload.tshirt_size })
            } 
          : item
      )
    );
...
>
        ...
        {(userRole === "master_admin" || userRole === "super_admin") && !attendee.is_archived && (
          ...
          >
            <FaArchive style={{ fontSize: "12px" }} /> Archive Record
          </button>
        )}
        ...
        {userRole === "master_admin" && attendee.is_archived && (
          ...
          >
            <FaArchive style={{ fontSize: "12px", transform: "rotate(180deg)" }} /> Restore Record
          </button>
        )}
      </div>
    </>
        ...
        <div className={styles.formRow}>
          ...
          <div className={styles.formGroup}>
            ...
            <input 
              ...
              onChange={(e) => handleEditFieldChange("middle_name", e.target.value)} 
            />
          </div>
          <div className={styles.formGroup}>
