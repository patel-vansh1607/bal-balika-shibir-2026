...
export default function CameraScanner({
  ...
  const currentTheme = scanResult
    ? statusThemes[scanResult.status] || statusThemes.success
    : statusThemes.success;
  ...
  const startCameraEngineDirectly = useCallback(async () => {
    ...
    try {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        try {
          const preemptiveStream = await navigator.mediaDevices.getUserMedia({
      ...
      await scanner.start(
        ...
        async (decodedText) => {
          ...
          if (
            scannedId.startsWith("MTRC-") &&
            !scannedId.includes("-KE-") &&
            !scannedId.includes("-TZ-") &&
            !scannedId.includes("-UG-") &&
            !scannedId.includes("-ZM-") &&
            !scannedId.includes("-MW-") &&
            !scannedId.includes("-BW-") &&
            !scannedId.includes("-ZA-")
          ) {
          ...
          if (fetchErrorInstance || !attendeeRecord) {
            ...
            setScanResult({
              status: "error",
              message: `Badge Unrecognized. Token "${scannedId}" not registered.`,
            });
          ...
          if (
            targetRegionScope !== "All" &&
            assignedRegion.toLowerCase() !== targetRegionScope.toLowerCase()
          ) {
          ...
          try {
            ...
            setScanResult({
              status: "success",
              message: "Checked In Approved!",
              attendee: attendeeRecord,
            });
            setScannerLog((prev) => [localLogPayload, ...prev]);
          } catch (writeErr) {
            console.error(
              "Failed executing admission payload state commitment:",
              writeErr,
            );
          } finally {
            setIsProcessing(false);
      ...
      setScanResult({
        ...
        customDetail:
          "Please confirm camera permissions are granted for this origin, and that the page is running on HTTPS.",
      });
    } finally {
      isStartingEngine.current = false;
    }
  ...
  return (
    <div className={styles.scannerWorkspaceGrid}>
      <div
        className={styles.mainCaptureCard}
        style={{
          position: "relative",
          overflow: "hidden",
          borderRadius: "12px",
      ...
      >
            ...
            >
              <div
                style={{
                  width: "60px",
                  height: "60px",
                  border: "3px solid rgba(138, 21, 27, 0.1)",
                  borderTop: "3px solid #8a151b",
                  borderRadius: "50%",
              ...
              <div
                style={{
                  ...
                  borderBottom: "3px solid #2d2926",
                  borderRadius: "50%",
                  animation: "spin-loader 1.2s infinite reverse linear",
                }}
              ></div>
            </div>
        ...
        {scanResult && !isProcessing && (
          <div
            className={`${styles.resultBannerCard} ${styles[scanResult.status]}`}
            style={{
              position: "absolute",
              top: 0,
              left: 0,
            ...
            <div style={{ width: "100%" }}>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {scanResult.customDetail ||
                      (sessionId
                        ? "Logged to Active Session"
                        : "Check In Complete")}
                  </p>
                </div>
              </div>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        {scanResult.attendee.age} Years Old
                      </span>
                    </div>
                    <div>
                      <span
        ...
        <div className={styles.cameraWrapperActive}>
          <div className={styles.streamScopeBanner}>
            Scanning exclusively for <strong>{prefixScope}</strong> identity
            passes...
          </div>
          ...
          <div
            className={styles.activeFenceBadge}
            style={{ marginTop: "12px", justifyContent: "center" }}
          >
            <FaShieldAlt /> Region:{" "}
            <strong>
              {regionScope === "All" ? "All Africa" : regionScope}
            </strong>
          </div>
        </div>
      </div>
      ...
      <div className={styles.auditLogPanelCard}>
        ...
        <div className={styles.logStreamTrackFeed}>
          ...
          {scannerLog.map((log) => (
            <div
              key={log.id}
              className={`${styles.logRowEntry} ${styles[`log_${log.type}`]}`}
            >
