...
export default function SelectRegion() {
  ...
  if (loading) return <div className={styles.regionWrapper}><FaSpinner className={`${styles.spin} ${styles.center}`} /></div>;
      ...
      <div className={styles.regionCard} style={{ maxWidth: "1150px" }}>
        ...
        <div className={styles.footerActionRow}>
          <button onClick={handleLogout} className={styles.logoutBtn} disabled={isLoggingOut}>
            {isLoggingOut ? <><FaSpinner className={styles.spin} style={{ marginRight: "6px" }} /> Logging out...</> : <><FaSignOutAlt style={{ marginRight: "6px" }} /> Logout</>}
          </button>
        </div>
      </div>
    </div>
