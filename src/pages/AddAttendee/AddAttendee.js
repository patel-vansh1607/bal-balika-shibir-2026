          ...
          canvas.toBlob(
            (blob) => {
              if (blob) {
                ...
                reject(
                  new Error(
                    "Canvas compression trace returned null raw stream.",
                  ),
                );
              }
            },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (insertData) {
        ...
        setTimeout(async () => {
          try {
            const qrPublicUrl = await uploadQRToSupabase(
              attendeeId,
              insertData.name,
            );
      ...
      <div className={styles.card}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className={styles.submitBtn} disabled={loading}>
            {loading ? (
              <>
                <FaSpinner className={styles.spin} /> Processing Assets...
              </>
            ) : (
              <>
                <FaUserPlus /> Save & Create Gate-Pass
              </>
            )}
          </button>
        </form>
        ...
        <div className={styles.badgeWrapper}>
          <div className={styles.badgeIdCard}>
            ...
            <div className={styles.badgeBodyContent}>
              <img
                src={finalAttendeeData.photoUrl}
                alt="Attendee"
                className={styles.badgeAvatarPhoto}
              />

              <div className={styles.badgeTextMeta}>
                <h3>{finalAttendeeData.name}</h3>
                <span className={styles.badgeCenterTag}>
                  {finalAttendeeData.center}
                </span>
                <span className={styles.badgeNumberToken}>
                  ID Token: #{finalAttendeeData.id}
                </span>
              </div>
          ...
          {downloadUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              download={`${finalAttendeeData.name.replace(/\s+/g, "_")}_qr.svg`}
